...
import { JobMediatorInput } from '../model/jobMediatorInput';
import { MediatorJob } from '../model/mediatorJob';
...
export class JobsApi {
     ...
     * @param jobMediatorInput Object describing the Job to be executed.
     */
    public async createJob (jobMediatorInput: JobMediatorInput, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: MediatorJob;  }> {
        ...
        // verify required parameter 'jobMediatorInput' is not null or undefined
        if (jobMediatorInput === null || jobMediatorInput === undefined) {
            throw new Error('Required parameter jobMediatorInput was null or undefined when calling createJob.');
        }
        ...
        let localVarRequestOptions: localVarRequest.Options = {
            ...
            json: true,
            body: ObjectSerializer.serialize(jobMediatorInput, "JobMediatorInput")
        };

        let authenticationPromise = Promise.resolve();
        if (this.authentications.tokenSignature.apiKey) {
            authenticationPromise = authenticationPromise.then(() => this.authentications.tokenSignature.applyToRequest(localVarRequestOptions));
        }
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: MediatorJob;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "MediatorJob");
    ...
    /**
     * Get job
     * @param jobId ID of the job as returned in JobMediatorEntity
     */
    public async getJobById (jobId: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: MediatorJob;  }> {
        const localVarPath = this.basePath + '/jobs/{jobId}'
