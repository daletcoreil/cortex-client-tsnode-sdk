...
export enum JobTypesApiApiKeys {
    tokenSignature,
}
...
export class JobTypesApi {
    ...
    public setApiKey(key: JobTypesApiApiKeys, value: string) {
        (this.authentications as any)[JobTypesApiApiKeys[key]].apiKey = value;
    }
     ...
     * @param projectServiceId Project service id of the client which associated to the request
     */
    public async getAllJobTypes (projectServiceId: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: Array<JobType>;  }> {
        const localVarPath = this.basePath + '/job-types';
        ...
        if (projectServiceId === null || projectServiceId === undefined) {
            throw new Error('Required parameter projectServiceId was null or undefined when calling getAllJobTypes.');
        }

        localVarHeaderParams['ProjectServiceId'] = ObjectSerializer.serialize(projectServiceId, "string");
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: Array<JobType>;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "Array<JobType>");
